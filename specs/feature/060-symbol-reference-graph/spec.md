...
- 言語サーバーがタイムアウトした場合 (3秒以内に応答なし): 「Request timed out」と表示し、再試行オプション
...
- **FR-031**: 言語サーバーのリクエストでシンボル参照 (US1) と呼び出し階層 (US2) を取得する
- **FR-032**: 言語サーバー起動失敗、タイムアウト (3秒) 時はユーザーフレンドリーなエラーメッセージを表示する
...
- 言語サーバーがインストールされている (または利用可能、未インストール時は graceful degradation)
- 外部グラフ描画ツールは任意 (未インストール時はテキストフォールバック)
