...
**Created**: 2026-01-30
**Status**: Draft
**Input**: Phase 4.0 - シンボル参照グラフをプレビューに表示。LSP (zls) を使用してシンボルの参照関係をグラフとして可視化。
...
1. **Given** Zig ファイルで関数定義にカーソルがある, **When** ユーザーが `gr` (go to references) を押す, **Then** その関数を呼び出している箇所が一覧表示される
...
6. **Given** LSP が利用できない (zls 未インストール), **When** 参照検索を実行, **Then** 「Language server not available」と表示される
...
6. **Given** ターミナルがグラフィック表示に対応していない, **When** グラフ表示を試みる, **Then** テキストベースのツリー表示にフォールバック
...
1. **Given** 参照一覧が表示されている, **When** ユーザーが `f` を押す, **Then** フィルタ入力モードになる
...
**シンボル参照検索 (US1)**
- **FR-001**: `gr` キーでカーソル位置のシンボルの参照を検索する
...
- **FR-006**: LSP が利用できない場合は「Language server not available」を表示する

**グラフ表示 (US2)**
- **FR-010**: 参照一覧で `G` を押すとグラフ表示モードに切り替わる
- **FR-011**: グラフのノードにファイル名、行番号、コードスニペット (±3行) を表示する
- **FR-012**: グラフのエッジで呼び出し関係を表現する
- **FR-013**: グラフィック非対応ターミナルではテキストベースのツリー表示にフォールバックする
...
**LSP 連携**
- **FR-030**: LSP (zls) を外部プロセスとして起動し通信する
- **FR-031**: LSP の `textDocument/references` リクエストでシンボル参照を取得する
- **FR-032**: LSP 起動失敗、タイムアウト時はユーザーフレンドリーなエラーメッセージを表示する
- **FR-033**: LSP プロセスのライフサイクルを適切に管理する (起動、再起動、終了)

### Key Entities

- **SymbolReference**: 参照箇所 (ファイルパス、行番号、列番号、コードスニペット)
- **ReferenceGraph**: シンボル参照のグラフ構造 (ノード: SymbolReference, エッジ: 呼び出し関係)
- **LspClient**: LSP サーバーとの通信を管理するクライアント
...
- **SC-004**: LSP 未インストール時もアプリがクラッシュせず、適切なエラーメッセージを表示する
- **SC-005**: グラフィック非対応ターミナルでもテキストフォールバックで参照を確認できる
...
- LSP 設定のカスタマイズ (zls の設定ファイルはユーザーが別途管理)
- 複数言語の同時対応
...
- ユーザーは Zig プロジェクトで作業し、zls がインストールされている (または利用可能)
- ターミナルは Kitty Graphics Protocol または Sixel をサポートしている (フォールバックあり)
- グラフレイアウト計算には外部ツール (Graphviz) を使用する可能性がある
