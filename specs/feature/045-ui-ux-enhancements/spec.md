...
**ダブルクリック (US2)**
- **FR-010**: ダブルクリックを時間閾値（400ms）で検出する（monotonic time 使用）
- **FR-011**: ディレクトリのダブルクリックで展開/折りたたみをトグルする
- **FR-012**: ファイルのダブルクリックでプレビューモードを開く
- **FR-013**: ダブルクリック検出には同一エントリ（visible index + scroll offset）のクリックであることを確認する
...
- **FR-025**: ヘルプヒント `?:help` は常に表示する
- **FR-026**: stat 失敗時はサイズ/日時に "-" を表示する
- **FR-027**: 空のツリーまたはカーソルがエントリ上にない場合、パスのみ表示する
...
- **FR-035**: アイコン幅は vaxis の実際のセル幅計測を使用する（固定幅仮定しない）
...
- **SC-001**: マウスクリックからカーソル移動まで 50ms 以内に反応する
- **SC-002**: ダブルクリック検出の精度が 99% 以上（誤検出なし）
- **SC-003**: ステータスバーの情報更新がカーソル移動と同期する（遅延なし）
- **SC-004**: 主要なファイル形式（20 種類以上）のアイコンをサポートする
